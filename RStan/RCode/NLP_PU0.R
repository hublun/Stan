library("rstan")
library("parallel")
#======================= threading and avoid recompile ========================
options(mc.cores = parallel::detectCores())
rstan_options(auto_write = TRUE)
Sys.setenv(LOCAL_CPPFLAGS = '-march=native')
#===================== Working Directory ======================================
getwd()
setwd("~")
remove(list = ls()) # clear the current work space
#========================Processing input data ================================
#NLP741X1138Hits$hits
#df_NLP = NLP741X1138Hits
#saveRDS(object=df_NLP, file="~/Dropbox/Stan/Data/NLP_DF")
df <- readRDS(file="~/Dropbox/Stan/Data/NLP_DF")
df

  df$u <- as.integer(df$uid)
  df$p = as.integer(df$pid)
  df$y = as.integer(df$hits)

dl <- list(N = length(df$y), y = df$y, P=length(unique(df$pid)),U=length(unique(df$uid)) ,xp=df$p, xu=df$u)
dl

  lambda_p <- aggregate.data.frame(x=df$y, by=list(df$p), FUN=mean)
  sigma_p  <- aggregate.data.frame(x=df$y, by=list(df$p), FUN=sd)
  sigma_p$x[is.na(sigma_p$x)] <- 0.01

  lambda_u <- aggregate.data.frame(x=df$y, by=list(df$u), FUN=mean)
  sigma_u  <- aggregate.data.frame(x=df$y, by=list(df$u), FUN=sd)
  sigma_u$x[is.na(sigma_u$x)] <- 0.01
  
initValues <- list(muP = log(mean(lambda_p$x)), muU= log(mean(lambda_u$x)), mu_p =log(lambda_p$x), mu_u = log(lambda_u$x), tau_p = mean(sigma_p$x), tau_u=mean(sigma_u$x))
initValues

#============================================ Model ======================================================
remove(sm)
sm = stan_model("~/Dropbox/Stan/Models/NLP_PU0.stan")
#=========================================================================================================
remove(fit)
fit = sampling(object = sm, data=dl, 
                init=list(initValues), 
                control=list(adapt_delta = 0.95), chains=1, iter=1000, warmup=500, thin=1)
#====================================== Plotting =========================================================
traceplot(fit, pars=c("muP","muU","tau_p","tau_u"))
pairs(fit, pars=c("muP","tau_p","muU","tau_u"))
print(fit)
plot(fit, pars = c("mu_p[1]","mu_p[741]", "mu_u[1]","mu_u[1138]"), show_density=FALSE)
plot(fit, pars = c("yp[1]","yp[2]","yp[3]","yp[4]","yp[5]","yp[12345]"), show_density=FALSE)
summary(fit)

sims = extract(fit, permuted=TRUE)
#=========================================================
class(sims$yp)
mtx = sims$yp
nrow(mtx) # iterations
ncol(mtx) # number of parameters

yp <- mtx[1,]
y <- df$y

mse_l=c()
class(mse_l)
for (i in 1:nrow(mtx)){
  mse_l <- c(mse_l, mse(df$y,mtx[i,]))
}
mse_l

max(mtx)
min(mtx)
mean(mtx)
median(mtx)
max(mtx[,2])
#=============================================================
